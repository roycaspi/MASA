...
export function getData(id) {
    const q = query(eventsCollection, where('user', '==', id)); 
    ...
    userDocSnapShot.data().Data.forEach(async ref => { //get appointments from appointments refrences
        appointmentDocRef = doc(db,  ref)
        appointmentDocSnap = await getDoc(appointmentDocRef)
...
export function editEvent(changed, currentUser) {
    try{
        ...
        if(coli){ 
          ...
          toChange.participants.forEach(async p =>{//update event for all current participants
            const q = query(eventsCollection, where('user', "==", p))
            const partiDocSnap = await getDocs(q);
            const partiDocRef = doc(db, partiDocSnap.docs[0].Pointer)
          ...
          if(changedDetails.participants){ //the participants field changed
            ...
            eventParticipants.forEach(async p => { //delete from participants that got deleted
              if(!changedDetails.participants.includes(p)){
                ...
                const partiDocSnap = await getDocs(q);
                const partiDocRef = doc(db, partiDocSnap.docs[0].Pointer)
                const docSnapShot = await getDoc(partiDocRef);
                updateDoc(partiDocRef, {
                data: docSnapShot.docs[0].data().data.filter(appointment => appointment.id != Object.keys(changed)[0]) });
              }
            })
          }
        }
...
export function removeEvent(deleted, currentUser) {
